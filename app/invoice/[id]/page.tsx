    ...
    return (
      <div className="min-h-screen bg-[#FCFCFA] flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <div className="w-12 h-12 bg-[#8C1D2F] rounded-full flex items-center justify-center">
  ...
  if (error || !order) {
    return (
      <div className="min-h-screen bg-[#FCFCFA] flex flex-col items-center justify-center gap-4">
        <p className="text-[#8C1D2F] font-bold text-xl">Invoice Not Found</p>
        <Link href="/" className="px-6 py-2 bg-[#E8DEC8] hover:bg-[#DBCABF] rounded-lg text-[#32231A] font-bold transition-colors">
          Return to Dashboard
        </Link>
      </div>
    );
      ...
      <div className="w-full max-w-3xl flex justify-end items-center mb-8 print:hidden gap-4">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="flex items-center gap-2 bg-white hover:bg-[#F0EBE1]/40 text-[#4C3D32] hover:text-[#8C1D2F] font-bold text-xs uppercase tracking-wider px-4 py-2 rounded-lg shadow-sm border border-[#E8DEC8] transition-colors cursor-pointer"
          >
            {copied ? (
              <>
          ...
          <button 
            onClick={handlePrint}
            className="flex items-center gap-2 bg-[#8C1D2F] hover:bg-[#6B1422] text-white font-bold text-xs uppercase tracking-wider px-5 py-2 rounded-lg shadow-sm transition-colors cursor-pointer"
          >
      ...
      <div className="w-full max-w-3xl bg-white border border-[#E8DEC8] rounded-2xl shadow-xl print:shadow-none print:border-none print:rounded-none overflow-hidden">
        ...
        <div className="bg-[#FCFCFA] border-b border-[#E8DEC8] p-8 sm:p-12 print:p-6 flex flex-col items-center text-center">
          <div className="w-24 h-24 flex items-center justify-center mb-4">
            <img src="/logo.png" alt="UPDATE MENS WEAR Logo" className="max-w-full max-h-full object-contain" />
          </div>
          <h1 className="text-3xl font-black text-[#192836] tracking-tight ">UPDATE MENS WEAR</h1>
            <p className="text-xs text-[#8C1D2F] font-bold tracking-wider mt-1 mb-4">INVOICE: {order.id}</p>
          ...
          <div className="flex flex-col items-center gap-2 text-sm text-[#4C3D32] font-semibold">
            <div className="text-center max-w-md leading-relaxed">
              <span className="inline-block text-[#8C1D2F] mr-1.5 align-middle -mt-0.5">
                <MapPin className="w-3.5 h-3.5" />
              </span>
              <span>131, Eldams Rd, Subbarayan Nagar, Teynampet, Chennai, Tamil Nadu 600018</span>
            </div>
            <div className="flex items-center gap-1.5 justify-center">
              <Phone className="w-3.5 h-3.5 text-[#8C1D2F] shrink-0" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 p-8 sm:p-12 print:p-6 border-b border-[#E8DEC8]/50">
          <div>
            ...
            <p className="text-base font-bold text-[#192836]">{order.customers?.name || "Guest Customer"}</p>
          ...
          <div className="sm:text-right flex flex-col sm:items-end">
            ...
            <div className="inline-block text-left text-sm space-y-1">
              <div className="flex gap-2">
                ...
                <span className="text-[#32231A] font-black">{new Date(order.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              </div>
              <div className="flex gap-2">
                <span className="text-[#8A7B72] font-bold w-12 text-left sm:text-right">Time:</span>
                <span className="text-[#32231A] font-black">{new Date(order.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
              <div className="flex gap-2">
              ...
              {order.order_items.map((item: any, index: number) => (
                <tr key={index} className="group">
                  <td className="py-6 pr-4 print:py-3">
                    <p className="text-sm font-bold text-[#192836]">{item.snapshot_name}</p>
                  </td>
                  ...
                  <td className="py-6 pl-4 print:py-3 text-right text-sm font-black text-[#192836]">₹{(item.snapshot_price * item.quantity).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <div className="w-full sm:w-1/2 space-y-3">
              {(order.discount_amount > 0 || order.delivery_fee > 0) && (
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold text-[#32231A]">₹{order.subtotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                </div>
              )}
              ...
              {order.delivery_fee > 0 && (
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold text-[#32231A]">₹{order.delivery_fee.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                </div>
              )}

              <div className="border-t border-[#E8DEC8] pt-4 mt-2 flex justify-between items-center">
                <span className="text-sm font-black text-[#8C1D2F] uppercase tracking-widest">Total Amount</span>
                <span className="text-3xl font-black text-[#192836]">₹{order.grand_total.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
            </div>
        </div>
        ...
        <div className="border-t border-[#E8DEC8]/60 p-6 print:p-4 text-center bg-[#F8F5F0] flex flex-col items-center justify-center gap-1.5">
          <p className="text-xs font-bold text-[#8C1D2F] tracking-wider uppercase">Thank you for shopping!</p>
          <p className="text-[9px] font-bold text-[#8A7B72]/80 uppercase tracking-[0.15em]">Powered by cenexa system @2026</p>
        </div>
